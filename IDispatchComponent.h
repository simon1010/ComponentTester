        ...
        : mc_sCompID([]{sv_nCompNr++;return std::to_string(sv_nCompNr);}())
{
  ...
  for(int i = 0; i < ac_nPortCount; ++i) {
    mv_Ports[i].mv_sCurrent_IN  = ([this, i] { return mc_sCompID + "_" + std::to_string(i) + "_II_"; }());
    AddInput_ (mv_Ports[i].mv_sCurrent_IN); // Current in

    mv_Ports[i].mv_sCurrent_OUT = ([this, i] { return mc_sCompID + "_" + std::to_string(i) + "_IO_"; }());
    AddOutput_(mv_Ports[i].mv_sCurrent_OUT); // Current source

    mv_Ports[i].mv_sVoltage_IN  = ([this, i] { return mc_sCompID + "_" + std::to_string(i) + "_UI_"; }());
    AddInput_ (mv_Ports[i].mv_sVoltage_IN); // Voltage in

    mv_Ports[i].mv_sVoltage_OUT = ([this, i] { return mc_sCompID + "_" + std::to_string(i) + "_UO_"; }());
